...
while (~done)
  ...
  if (largescale)
    ...
    %%% Mikko modified cond check %%%%%
    isillcond_mikko = false;
    if isnan(cond(H11p,1))
        isillcond_mikko = true;
    else
        [dv,hcond] = linsolve(H11p, w1p, opts); %% Originally only this
    end
    
    if (hcond < 1e-14) || isillcond_mikko %%% Check added here
